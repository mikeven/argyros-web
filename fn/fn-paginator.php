<?php 
	...
	/* ----------------------------------------------------------------------------------- */
	function tituloProductoNoDisponible(){
		//
		return "<h3 id='page-title' class='text-left'>
					<span itemprop='name' style='color: #a7b239;'> PRODUCTO NO DISPONIBLE </span>
				</h3>";
	}
	/* ----------------------------------------------------------------------------------- */
	function obtenerBloqueTallasAsignadas( $datos_p, $iddet, $tallas ){
		...
		if( $disponible == false ) $bloque_tallas = tituloProductoNoDisponible();
	...
	function obtenerElementoCatalogo( $purl, $item, $numerador ){
		...
		if( ( $item["d_antiguedad"] < NDIAS_NUEVO ) || ( $item["d_reposicion"] < NDIAS_NUEVO && $item["d_reposicion"] != NULL ) )
