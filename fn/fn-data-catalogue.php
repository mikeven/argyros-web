<?php 
	...
	/* ----------------------------------------------------------------------------------- */
	function matchFiltroAtributoDetalle( $producto, $atributo, $valores_filtro ){
	...
	/* ----------------------------------------------------------------------------------- */
	function matchFiltroPrecio( $dbh, $reg, $atr, $vals_filtro, $ft, $ver_ocultos ){
		...
		if( $atr == "precio_pieza" ){
			...
			foreach ( $reg["sizes"] as $rt ) {
				/*if( $reg["iddet"] == 4164 ){
					echo "IDD: ".$reg["iddet"].">>> PRE: ".$rt["precio"]." [$rt[talla]] "."<(".$vals_filtro[0]."-".$vals_filtro[1].")>"."\n";
					echo "EVAL PRE>=mín: ".( $rt["precio"] >= $vals_filtro[0] )." ";
					echo "EVAL PRE<=máx: ".( $rt["precio"] <= $vals_filtro[1] )." "."\n";;
				}*/
				
				if( ( $rt["precio"] >= $vals_filtro[0] ) && ( $rt["precio"] <= $vals_filtro[1] ) ){
					
					if( $ver_ocultos == true && $rt["visible"] == 0 )
						$match = true;			//Match por tipo de precio: pieza - talla oculta
					if( $ver_ocultos == false && $rt["visible"] == 1 )
						$match = true;			//Match por tipo de precio: pieza - talla disponible
					break;
	...
	function filtrarProductosPorAtributoDetalleProducto( $productos, $param, $vals_filtro ){
		...
		foreach ( $productos as $p ){
			...
			if( matchFiltroAtributoDetalle( $p, $atributo, $vals_filtro ) )
				$filtrados[] = $p;
		}
	...
	/* ----------------------------------------------------------------------------------- */
	function filtrarProductosPorPrecio( $dbh, $productos, $atr, $vals_filtros, $ft, $ver_ocultos ){
		...
		foreach ( $productos as $prod ){
			...
			if( matchFiltroPrecio( $dbh, $prod, $cmp, $vals_filtros, $ft, $ver_ocultos ) )
				$filtrados[] = $prod;
		}
	...
	function obtenerProductosListaCatalogo( $dbh, $catalogue_url, $url_params ) {
		...
		/*..........................................................................*/
		if( isset( $url_params[P_TEXTO_BUSQUEDA] ) && $url_params[P_TEXTO_BUSQUEDA] != "" ){
